...
- **Multiple conversation sequences**: onboarding, tutorial, support, main menu, autoroute test, and custom value demo
...
- **Assets**: 
  ...
  - **Sequences**: Modular JSON files in `assets/sequences/` containing:
    ...
    - `custom_value_demo.json` - Demonstration of custom value storage in choices
  - Each sequence supports branching conversations, choices, text input, cross-sequence navigation, autoroute conditions, custom value storage, and storage keys
...
### Custom Value Storage System (Latest Feature)

The custom value storage system enables **clean data storage separate from display text**. Users see friendly choice text while the system stores professional values for conditions and data processing.

#### **Custom Value Choice Structure**
```json
{
  "id": 10,
  ...
  "choices": [
    {
      "text": "I'm just getting started with this",
      "value": "beginner",
      "nextMessageId": 11
    },
    {
...
- `value`: Custom value to store (can be string, boolean, number, null)
- `text`: Display text shown to users (unchanged)
- **Fallback behavior**: If no `value` provided, stores `text` (backward compatible)

#### **Supported Value Types:**
- **Strings**: `"beginner"`, `"premium"`, `"developer"`
- **Booleans**: `true`, `false`
- **Numbers**: `0`, `5`, `10.5`
- **Null**: `null`

#### **Key Benefits:**
- ✅ **Clean Data Storage** - Professional values instead of messy display text
- ✅ **Better Conditions** - `user.level == 'beginner'` vs `user.level == 'I\\'m just getting started'`
- ✅ **Internationalization Ready** - UI text separate from stored values
- ✅ **Type Flexibility** - Store booleans, numbers, strings, or null values
- ✅ **Backward Compatible** - Existing sequences work unchanged

#### **Implementation Files:**
- `lib/models/choice.dart` - Extended with `value` field and JSON serialization
- `lib/services/chat_service.dart` - Updated `handleUserChoice()` to use custom values
- `lib/widgets/chat_screen/chat_state_manager.dart` - Passes Choice object instead of text
- `assets/sequences/custom_value_demo.json` - Working demonstration sequence
...
```json
{
  ...
  "storeKey": "notifications_enabled",
  "choices": [
...
```json
{
  ...
  "storeKey": "user_role",
  "choices": [
    {"text": "👨‍💻 Software Developer", "value": "developer"},
    {"text": "🎨 UI/UX Designer", "value": "designer"},
...
#### **Testing the Custom Value System:**
1. **Access Debug Panel** - Tap bug icon (🐛) in app bar
2. **Select "Custom Value Demo"** - Choose from sequence dropdown (settings icon ⚙️)
3. **Make choices** - See friendly display text in conversation
4. **Check stored values** - Debug panel shows clean stored values
5. **Observe autoroute** - Routing based on clean stored values
...
- **Unified Sequence Switching**: Consistent state management for both choice-based and autoroute-based navigation
...
- **Sequence Selection**: Dropdown selector in debug panel for easy sequence switching (includes autoroute and custom value tests)
- **Modular Conversations**: Organized conversation flows for onboarding, tutorials, support, navigation, conditional routing, and custom value demos
...
12. **Test autoroute system**: Select "Autoroute Test" (🛤️) to see conditional routing in action
13. **Test custom values**: Select "Custom Value Demo" (⚙️) to see clean data storage
14. **Enhanced UX**: Notice questions displayed with choice buttons and text input fields
15. Begin development by following TDD principles and updating relevant models, services, or widgets
16. **Sequence system**: Add new conversation flows by creating JSON files in `assets/sequences/`
17. **Modular architecture**: Components are organized in `lib/widgets/chat_screen/` for easy maintenance
18. **Custom values**: Use `value` field in choices for clean data storage separate from display text
19. **Unified switching**: Both choice-based and autoroute-based sequence switching use consistent state management
...
#### Unified Sequence Switching & Custom Values (Latest - December 2024)
- **Unified Sequence Switching**: Centralized sequence switching logic for both choice-based and autoroute-based navigation
- **Custom Value Storage**: Enhanced Choice model with custom value field for clean data storage
- **Enhanced Message Display**: Fixed UX issue where choice/input messages showed only interactive elements without context text
- **State Consistency**: Eliminated state management inconsistencies between different switching mechanisms
- **Debug Panel Integration**: Added custom value demo sequence with settings icon (⚙️) for easy testing
- **Callback System**: Implemented ChatService callback mechanism for autoroute → UI communication
- **Source Tracking**: Added source identification ('choice' vs 'autoroute') for debugging sequence switches
- **Backward Compatibility**: All existing sequences work unchanged, enhancements are purely additive

#### Unified Switching Features:
```
Sequence Switching Improvements:
├── Centralized Logic
│   ├── Single _switchSequence() method in ChatStateManager
│   ├── Proper state management for both switching types
│   └── Consistent error handling and logging
├── Custom Value Storage
│   ├── Choice.value field with fallback to text
│   ├── Clean data storage (e.g., "beginner" vs "I'm just getting started")
│   └── Better autoroute conditions and internationalization support
├── Enhanced UX
│   ├── Question text + choice buttons displayed together
│   ├── Prompt text + input field displayed together
│   └── Professional appearance with proper spacing
└── Debug Integration
    ├── Custom value demo sequence
    ├── Real-time state updates after autoroute switches
    └── Clean data display in debug panel
```

#### Autoroute System Implementation (December 2024)
...
### Comparison Operators Enhancement (December 2024)

The ConditionEvaluator has been enhanced with numeric comparison operators:

#### New Operators Added:
- Greater than: user.age > 18
- Less than: user.score < 100  
- Greater than or equal: user.experience >= 5
- Less than or equal: user.rating <= 4.5

#### Key Features:
- Numeric type support for integers and doubles
- Automatic string-to-number conversion
- Mixed type comparisons (25 >= 24.5)
- Graceful error handling for non-numeric values
- Operator precedence (>= and <= parsed before > and <)
- Backward compatible with existing ==, !=, boolean conditions

#### Demo Sequence:
- comparison_demo.json added to assets/sequences/
- Interactive age-based and score-based routing examples
- Added to debug panel sequence selector
- Available in app constants and chat config

#### Usage Examples:
- Age verification: user.age >= 18
- Score thresholds: user.quiz_score > 80
- Experience levels: user.years_experience <= 2
- Rating limits: user.rating < 4.5

All existing functionality remains unchanged. The enhancement adds 12 comprehensive tests covering all comparison scenarios and edge cases.
