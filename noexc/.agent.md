...
This project uses `rive: 0.14.0-dev.5` and the new `RiveNative` runtime. Ensure `await RiveNative.init()` is called before usage (done in `main.dart`). Use `Factory.rive` when creating files/images.

- Artboard selection:
  - Use `ArtboardSelector.byDefault()`, `.byName(name)`, or `.byIndex(i)` when creating `RiveWidgetController` (or via `RiveWidgetBuilder`).
  - Manually: `file.defaultArtboard()`, `file.artboard('Name')`, `file.artboardAt(i)`.

- State machine selection:
  - Prefer `RiveWidgetController(riveFile, stateMachineSelector: StateMachineSelector.byName('State Machine 1'))`.
  - Alternatives: `.byIndex(i)` or default.

- Layout and fit:
  - Support `Fit.layout` for responsive Layouts authored in Rive; optional `layoutScaleFactor`.
  - Otherwise, use `fit` + `alignment` (`Fit.contain` default). `Alignment` is ignored for `Fit.layout`.

- Data Binding (View Models):
  - Access view models from `file`: by name/index, or default for an artboard.
  - Create instances: blank/default/byName/byIndex. Bind via `controller.dataBind(DataBind.auto())` or compose manually.
  - Property types supported: number, bool, string, color, trigger, enum, nested view models, image, lists.
  - Nested paths: use slash-delimited paths e.g. `vmi.number('Nested/Prop')`.
  - Observe changes: properties support `addListener/removeListener/clearListeners` and must be disposed.

- Assets and images:
  - Embedded/Hosted/Referenced assets supported. For referenced assets, use `assetLoader:` in `File.asset(...)` to supply bytes and return `true` when handled.
  - Runtime image updates: `Factory.rive.decodeImage(bytes)` then assign to `viewModelInstance.image('name').value`.

- Caching Rive files:
  - You may keep a `File` alive and reuse it across widgets to avoid repeated decoding. Dispose when no longer needed.

### Current Implementation Snapshot (App)

- Bubbles: `MessageBubble` wraps `_RiveAnimationWrapper` which loads a `File.asset(..., riveFactory: Factory.rive)` and displays via `RiveWidget(RiveWidgetController(file))`. It keeps state alive and disposes correctly. No artboard/state machine selection or data binding yet.
- Overlays: `RiveOverlayService` + `RiveOverlayHost` manage multiple overlays (zones, ids, z-index, queue/replace/ignore). Overlays support data binding via `controller.dataBind(DataBind.auto())` but currently only map numeric properties.
- Initialization: `await RiveNative.init()` is called in `main.dart`.

### Known Gaps vs Docs

- No artboard/state machine selection in bubbles or overlays; defaults only.
- No bubble-level data binding; overlays only, numeric-only.
- No support yet for bool/string/color/image/list bindings.
- No assetLoader path for referenced assets; not exposing `RiveWidgetBuilder` conveniences.
- No central `Rive File` cache; each usage loads a new file.
- Overlays don’t explicitly trigger a repaint when the controller becomes ready (host relies on incidental rebuilds).

### Recommendations

- Extend models/APIs to accept optional `artboard`, `stateMachine` (or `animation`), `dataModel`, and `bindings` (support number/bool/string/color initially).
- Bubbles: add optional bindings support and apply before first frame.
- Overlays: support full property types; keep numeric fast path for tests.
- Add simple `RiveAssetCache` service to reuse `File` objects across instances.
- Consider `RiveWidgetBuilder` for unified file loading and to surface `assetLoader` for referenced assets.
- For authored Layouts, allow `Fit.layout` + `layoutScaleFactor` as an option; otherwise continue with current `fit`/wrapper `Alignment`.
- Debug: on selection/binding failure, log available artboards/state machines/view models and property names via `LoggerService` (use quiet helpers in tests).
