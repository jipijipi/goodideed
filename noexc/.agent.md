...
- `lib/` - Main Dart source code
  ...
  - `services/` - Business logic services (ChatService)
  - `widgets/` - UI components (ChatScreen)
...
### Architecture
- **Models**: 
  - `ChatMessage` class for message data structure with sender, choices, and text input support
  - `Choice` class for representing user choice options
- **Services**: `ChatService` for loading chat script, managing conversation flow, and handling user interactions
- **Widgets**: `ChatScreen` for displaying the interactive chat interface with choice buttons and text input
- **Assets**: JSON file containing branching chat script with choices and text input prompts
...
3. Use `flutter test` to verify all tests pass
4. Use `flutter run` to start the interactive chat application
5. Explore the chat interface with choice-based navigation and text input
6. Begin development by following TDD principles and updating relevant models, services, or widgets
