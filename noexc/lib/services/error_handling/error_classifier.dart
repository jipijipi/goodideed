...
class ErrorClassifier {
  ...
  static Exception handleSequenceLoadError(String sequenceId, dynamic error) {
    final message =
        'Failed to load sequence "$sequenceId": ${error.toString()}';
  ...
  static Exception handleTemplateError(String template, dynamic error) {
    final message =
        'Template processing failed for "$template": ${error.toString()}';
    _logError(message, error);
  ...
  static Exception handleConditionError(String condition, dynamic error) {
    final message =
        'Condition evaluation failed for "$condition": ${error.toString()}';
    _logError(message, error);
    ...
    if (kDebugMode) {
      if (error is Exception) {
        logger.debug(
          ...
          component: LogComponent.errorHandler,
        );
      }
      if (error is Error) {
        logger.debug(
