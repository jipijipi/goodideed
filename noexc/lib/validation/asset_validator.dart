...
class AssetValidator {
  ...
  final CrossReferenceValidator _crossReferenceValidator =
      CrossReferenceValidator();
  final JsonSchemaValidator _jsonSchemaValidator = JsonSchemaValidator();
  final TemplateVariableValidator _templateVariableValidator =
  ...
  Future<ValidationResult> validateAllSequenceFiles() async {
    ...
    final sequenceResult =
        await _sequenceFileValidator.validateAllSequenceFiles();
    ...
    for (final sequenceId in AppConstants.availableSequences) {
      ...
      if (sequence != null) {
        final variantIssues = await _variantFileValidator.validateVariantFiles(
          sequence,
        );
    ...
    final crossRefIssues =
        await _crossReferenceValidator.validateCrossSequenceReferences();
  ...
  Future<ValidationResult> validateAllAssets() async {
    ...
    final templateIssues =
        await _templateVariableValidator.validateTemplateVariables();
    warnings.addAll(templateIssues);

    // 3. Template syntax validation
    final syntaxIssues =
        await _templateVariableValidator.validateTemplateVariableSyntax();
    warnings.addAll(syntaxIssues);

    // 4. Cross-reference accessibility validation
    final accessibilityIssues =
        await _crossReferenceValidator.validateSequenceAccessibility();
    ...
    for (final sequenceId in AppConstants.availableSequences) {
      final sequence = await _sequenceFileValidator.loadSequence(sequenceId);
      if (sequence != null) {
        final variantContentIssues = await _variantFileValidator
            .validateVariantContent(sequence);
