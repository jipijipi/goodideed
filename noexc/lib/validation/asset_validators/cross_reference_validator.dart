        ...
        if (message.choices != null) {
          for (final choice in message.choices!) {
            if (choice.sequenceId != null &&
                !availableSequences.contains(choice.sequenceId)) {
              errors.add(
                ValidationError(
                  type: 'INVALID_SEQUENCE_REFERENCE',
                  message:
                      'Choice references non-existent sequence: ${choice.sequenceId}',
        ...
        if (message.routes != null) {
          for (final route in message.routes!) {
            if (route.sequenceId != null &&
                !availableSequences.contains(route.sequenceId)) {
              errors.add(
                ValidationError(
                  type: 'INVALID_SEQUENCE_REFERENCE',
                  message:
                      'Route references non-existent sequence: ${route.sequenceId}',
                  messageId: message.id,
                  sequenceId: sequence.sequenceId,
                ),
              );
      ...
      if (sequence.sequenceId != sequenceId) {
        warnings.add(
          ValidationError(
            ...
            message:
                'Sequence file $sequenceId contains sequenceId "${sequence.sequenceId}"',
            sequenceId: sequenceId,
            severity: 'warning',
          ),
        );
