...
class JsonSchemaValidator {
  ...
  Future<ValidationResult> validateJsonSchema(String sequenceId) async {
      ...
      if (jsonData['messages'] is List) {
        ...
        for (int i = 0; i < messages.length; i++) {
          ...
          if (messageType == 'choice') {
            if (!messageMap.containsKey('choices') ||
                messageMap['choices'] is! List) {
              errors.add(
                ValidationError(
          ...
          if (messageType == 'autoroute') {
            if (!messageMap.containsKey('routes') ||
                messageMap['routes'] is! List) {
              errors.add(
                ValidationError(
                  type: 'MISSING_ROUTES',
            ...
            if (!messageMap.containsKey('storeKey')) {
              warnings.add(
                ValidationError(
                  ...
                  message:
                      'TextInput message should have "storeKey" for data storage',
                  messageId: messageMap['id'] as int?,
                  sequenceId: sequenceId,
                  severity: 'warning',
                ),
    ...
    return ValidationResult(errors: errors, warnings: warnings, info: []);
  }

  /// Validates optional fields for best practices
  void _validateOptionalFields(
    ...
    if (!jsonData.containsKey('description') ||
        ...
        jsonData['description'].toString().isEmpty) {
      warnings.add(
        ValidationError(
