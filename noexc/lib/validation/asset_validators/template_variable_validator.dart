...
class TemplateVariableValidator {
  ...
  Future<List<ValidationError>> validateTemplateVariables() async {
    ...
    final allVariables =
        <String, Set<String>>{}; // variable -> sequences using it
    ...
    for (final sequenceId in AppConstants.availableSequences) {
      ...
      for (final message in sequence.messages) {
        ...
        if (message.placeholderText.isNotEmpty) {
          final placeholderVars = _extractTemplateVariables(
            message.placeholderText,
          );
          for (final variable in placeholderVars) {
            allVariables
                .putIfAbsent(variable, () => <String>{})
                .add(sequenceId);
          }
        }
      }
      ...
      if (entry.value.length > 1) {
        warnings.add(
          ValidationError(
            ...
            message:
                'Template variable "${entry.key}" is used in multiple sequences: ${entry.value.join(', ')}',
            severity: 'info',
          ),
        );
      }
  ...
  Future<List<ValidationError>> validateTemplateVariableSyntax() async {
    ...
    for (final sequenceId in AppConstants.availableSequences) {
      ...
      for (final message in sequence.messages) {
        final templateErrors = _validateTemplateSyntax(
          message.text,
          message.id,
          sequenceId,
        );
        ...
        if (message.placeholderText.isNotEmpty) {
          final placeholderErrors = _validateTemplateSyntax(
            message.placeholderText,
            message.id,
            sequenceId,
          );
  ...
  List<ValidationError> _validateTemplateSyntax(
    String text,
    int messageId,
    String sequenceId,
  ) {
