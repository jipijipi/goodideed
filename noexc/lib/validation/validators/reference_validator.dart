        ...
        if (!messageIds.contains(message.nextMessageId)) {
          errors.add(
            ValidationError(
              ...
              message:
                  'References non-existent message ID: ${message.nextMessageId}',
      ...
      if (message.choices != null) {
        for (final choice in message.choices!) {
          if (choice.nextMessageId != null &&
              !messageIds.contains(choice.nextMessageId)) {
            errors.add(
              ValidationError(
                type: ValidationConstants.invalidChoiceNextMessageId,
                message:
                    'Choice "${choice.text}" references non-existent message ID: ${choice.nextMessageId}',
                messageId: message.id,
                sequenceId: sequence.sequenceId,
              ),
            );
      ...
      if (message.routes != null) {
        for (final route in message.routes!) {
          if (route.nextMessageId != null &&
              !messageIds.contains(route.nextMessageId)) {
            errors.add(
              ValidationError(
                type: ValidationConstants.invalidRouteNextMessageId,
                message:
                    'Route references non-existent message ID: ${route.nextMessageId}',
                messageId: message.id,
                sequenceId: sequence.sequenceId,
              ),
            );
