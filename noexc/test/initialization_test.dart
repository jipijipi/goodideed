...
void main() {
  ...
  group('App Initialization Sequence', () {
    ...
    test(
      'should complete SessionService initialization before ChatStateManager starts',
      () async {
        ...
        expect(
          isPastDeadline,
          isNotNull,
          reason: 'SessionService should compute isPastDeadline',
        );
        expect(
          isActiveDay,
          isNotNull,
          reason: 'SessionService should compute isActiveDay',
        );
        ...
        final finalIsPastDeadline = await userDataService.getValue<bool>(
          StorageKeys.taskIsPastDeadline,
        );
        final finalIsActiveDay = await userDataService.getValue<bool>(
          StorageKeys.taskIsActiveDay,
        );
        ...
        expect(
          finalIsPastDeadline,
          isNotNull,
          reason: 'Computed variables should persist after chat initialization',
        );
        expect(
          finalIsActiveDay,
          isNotNull,
    ...
    test(
      'should not start ChatStateManager until SessionService variables are computed',
      () async {
        ...
        final computedValue = await userDataService.getValue<bool>(
          StorageKeys.taskIsPastDeadline,
        );
        expect(
          computedValue,
          isNotNull,
          reason: 'SessionService should compute variables before chat starts',
        );
        ...
        expect(
          sessionServiceCompleted,
          isTrue,
          reason: 'SessionService should complete first',
        );
        expect(
          chatStateManagerStarted,
          isTrue,
          reason: 'ChatStateManager should start after SessionService',
        );

        // Verify variables remain available
        final finalComputedValue = await userDataService.getValue<bool>(
          StorageKeys.taskIsPastDeadline,
        );
        expect(
          finalComputedValue,
          isNotNull,
          reason: 'Computed variables should be available to chat service',
        );
      },
    );
  });
