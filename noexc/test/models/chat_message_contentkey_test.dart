...
void main() {
  group('ChatMessage ContentKey Support', () {
    ...
    test('should create copyWith method that handles contentKey', () {
      ...
      expect(copiedMessage.id, equals(originalMessage.id)); // Other fields preserved
    ...
    test('should preserve contentKey when expanding multi-text messages', () {
      ...
      expect(expandedMessages[2].contentKey, equals('bot.inform.multi_welcome.casual'));
    });
    ...
    test('should handle contentKey with choice messages', () {
      final choiceMessage = ChatMessage(
        ...
        choices: [
          Choice(text: 'Completed', contentKey: 'user.choose.task_status.completed'),
          Choice(text: 'Failed', contentKey: 'user.choose.task_status.failed'),
        ],
      );
      
      expect(choiceMessage.contentKey, equals('bot.request.task_status.gentle'));
      expect(choiceMessage.choices![0].contentKey, equals('user.choose.task_status.completed'));
      expect(choiceMessage.choices![1].contentKey, equals('user.choose.task_status.failed'));
    });
  });
}
