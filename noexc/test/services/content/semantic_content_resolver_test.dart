  ...
  group('SemanticContentResolver', () {
    group('_buildFallbackChain', () {
      ...
      test('should build correct fallback chain with multiple modifiers', () {
        List<String> chain = SemanticContentResolver.buildFallbackChain(
          'bot', 'acknowledge', 'task_completion', ['positive', 'first_time', 'celebratory']
        );
    ...
    group('_extractGenericSubject', () {
      test('should extract generic subjects from specific ones', () {
        expect(SemanticContentResolver.extractGenericSubject('task_completion'), equals('completion'));
        expect(SemanticContentResolver.extractGenericSubject('profile_save'), equals('save'));
        expect(SemanticContentResolver.extractGenericSubject('user_name'), equals('name'));
        expect(SemanticContentResolver.extractGenericSubject('deadline_selection'), equals('selection'));
        expect(SemanticContentResolver.extractGenericSubject('account_creation'), equals('creation'));
      });
      
      test('should return original subject if no generic pattern found', () {
        expect(SemanticContentResolver.extractGenericSubject('completion'), equals('completion'));
        expect(SemanticContentResolver.extractGenericSubject('randomword'), equals('randomword'));
      });
    });
    
    group('resolveContent', () {
      test('should return original text for invalid semantic keys', () async {
        String result = await SemanticContentResolver.resolveContent('invalid', 'original text');
        expect(result, equals('original text'));
        
        result = await SemanticContentResolver.resolveContent('bot.only', 'original text');
        expect(result, equals('original text'));
      });
    ...
    group('semantic key parsing', () {
      test('should parse semantic keys correctly', () {
        var parsed = SemanticContentResolver.parseSemanticKey('bot.acknowledge.completion.positive.first_time');
      ...
      test('should handle keys without modifiers', () {
        var parsed = SemanticContentResolver.parseSemanticKey('user.choose.option');
