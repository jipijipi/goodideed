...
import '../../lib/models/notification_permission_status.dart';
import '../../lib/models/notification_tap_event.dart';
  ...
  group('NotificationService', () {
    ...
    group('Phase 1: Permission Status Detection', () {
      group('getPermissionStatus', () {
        test('should return notRequested when no permission request has been made', () async {
        ...
        test('should return stored permission status when available', () async {
          await mockUserDataService.storeValue(StorageKeys.notificationPermissionStatus, 'granted');
          await mockUserDataService.storeValue(StorageKeys.notificationPermissionRequestCount, 1);

          final status = await notificationService.getPermissionStatus();
          expect(status, equals(NotificationPermissionStatus.granted));
        });

        test('should return unknown when permission was requested but no status stored', () async {
          await mockUserDataService.storeValue(StorageKeys.notificationPermissionRequestCount, 1);
          // No status stored
        ...
        test('should handle invalid stored permission status gracefully', () async {
          await mockUserDataService.storeValue(StorageKeys.notificationPermissionStatus, 'invalid_status');
          await mockUserDataService.storeValue(StorageKeys.notificationPermissionRequestCount, 1);
      ...
      group('permission tracking in requestPermissions', () {
        test('should increment request count when requesting permissions', () async {
          // Initial request count should be 0
          expect(await mockUserDataService.getValue<int>(StorageKeys.notificationPermissionRequestCount), isNull);
          ...
          final requestCount = await mockUserDataService.getValue<int>(StorageKeys.notificationPermissionRequestCount);
        ...
        test('should store last requested timestamp', () async {
          ...
          final lastRequestedString = await mockUserDataService.getValue<String>(StorageKeys.notificationPermissionLastRequested);
          expect(lastRequestedString, isNotNull);
          
          final lastRequested = DateTime.parse(lastRequestedString!);
          expect(lastRequested.isAfter(beforeTime) || lastRequested.isAtSameMomentAs(beforeTime), isTrue);
        });
    ...
    group('Phase 2: Notification Tap Handling', () {
      ...
      late List<NotificationTapEvent> capturedEvents;
      ...
      group('notification tap callback', () {
        test('should call AppStateService when notification is tapped', () async {
          ...
          expect(() => notificationService.setAppStateService(mockAppStateService), returnsNormally);
        });

        test('should support legacy callback system', () async {
          var callbackCalled = false;
          NotificationTapEvent? capturedEvent;
        ...
        test('should handle missing app state service gracefully', () async {
          ...
          expect(() => notificationService.setNotificationTapCallback((_) {}), returnsNormally);
        });
      });
      ...
      group('integration with AppStateService', () {
        ...
        test('should handle notification tap events through AppStateService', () async {
          await mockAppStateService.initialize();
          notificationService.setAppStateService(mockAppStateService);
    ...
    group('Phase 1 & 2 Integration', () {
      test('should work together - permission status and tap handling', () async {
        ...
        expect(permissionStatus, equals(NotificationPermissionStatus.notRequested));
        ...
        expect(permissionStatus, isNot(equals(NotificationPermissionStatus.notRequested)));
      ...
      test('should maintain data consistency across both phases', () async {
        ...
        await mockUserDataService.storeValue(StorageKeys.notificationPermissionStatus, 'granted');
        await mockUserDataService.storeValue(StorageKeys.notificationPermissionRequestCount, 2);
        ...
        final event = NotificationTapEvent(notificationId: 1001, type: NotificationType.dailyReminder);
        ...
        final requestCountAfter = await mockUserDataService.getValue<int>(StorageKeys.notificationPermissionRequestCount);
