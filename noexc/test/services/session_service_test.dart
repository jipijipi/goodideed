...
void main() {
  group('SessionService', () {
    ...
    test('should initialize session with visit count', () async {
      ...
      final visitCount = await userDataService.getValue<int>(
        'session.visitCount',
      );
      expect(visitCount, 1);
    });
    ...
    test(
      'should increment daily visit count on multiple initializations same day',
      () async {
        await sessionService.initializeSession();
        await sessionService.initializeSession();
    ...
    test('should always increment total visit count', () async {
      ...
      final totalVisitCount = await userDataService.getValue<int>(
    ...
    test('should set time of day correctly', () async {
      ...
      final timeOfDay = await userDataService.getValue<int>(
        'session.timeOfDay',
      );
    ...
    test('should set date information', () async {
      ...
      final daysSinceFirst = await userDataService.getValue<int>(
        'session.daysSinceFirstVisit',
      );
      final isWeekend = await userDataService.getValue<bool>(
        'session.isWeekend',
      );
    ...
    test('should preserve current date set by script', () async {
      ...
      final currentDate = await userDataService.getValue<String>(
        StorageKeys.taskCurrentDate,
      );
    ...
    test(
      'should preserve future date for next_active users across same-day sessions',
      () async {
        ...
        await userDataService.storeValue(
          StorageKeys.taskCurrentDate,
          tomorrowString,
        ); // Script sets future date

        // First session
        await sessionService.initializeSession();
        final firstCurrentDate = await userDataService.getValue<String>(
          StorageKeys.taskCurrentDate,
        );

        // Second session same day - should preserve the date set by script
        await sessionService.initializeSession();
        final secondCurrentDate = await userDataService.getValue<String>(
          StorageKeys.taskCurrentDate,
        );
    ...
    test(
      'should use session date for isNewDay calculation, not task date',
      () async {
        final tomorrow = DateTime.now().add(const Duration(days: 1));
        final tomorrowString = _formatDate(tomorrow);
        ...
        // First session
        await sessionService.initializeSession();
        final currentDate1 = await userDataService.getValue<String>(
          StorageKeys.taskCurrentDate,
        );

        // Second session same day - should preserve the script-set date
        await sessionService.initializeSession();
        final currentDate2 = await userDataService.getValue<String>(
          StorageKeys.taskCurrentDate,
        );
    ...
    test(
      'should store next active weekday when calculating future dates',
      () async {
        final tomorrow = DateTime.now().add(const Duration(days: 1));
        ...
        await userDataService.storeValue(
          StorageKeys.taskNextActiveWeekday,
          tomorrow.weekday,
        );

        await sessionService.initializeSession();

        final nextActiveWeekday = await userDataService.getValue<int>(
          StorageKeys.taskNextActiveWeekday,
        );
    ...
    test(
      'should return false for isActiveDay when today is not in activeDays',
      () async {
        final tomorrow = DateTime.now().add(const Duration(days: 1));
        final tomorrowString = _formatDate(tomorrow);
    ...
    test(
      'should return true for isActiveDay when today is in activeDays',
      () async {
    ...
    group('Active Day Pure Weekday Logic', () {
      ...
      test(
        'should return true for weekdays when activeDays includes weekdays',
        () async {
      ...
      test(
        'should return true for weekends when activeDays includes weekends',
        () async {
    ...
    group('Task Status Computation', () {
      test(
        'should compute task status as pending when task date equals today',
        () async {
          final today = DateTime.now();
          final todayString = _formatDate(today);
      ...
      test(
        'should compute task status as overdue when task date is before today',
        () async {
          final today = DateTime.now();
          final yesterday = today.subtract(const Duration(days: 1));
          final yesterdayString = _formatDate(yesterday);
      ...
      test(
        'should compute task status as upcoming when task date is after today',
        () async {
          final today = DateTime.now();
      ...
      test(
        'should default to pending when task current date is null',
        () async {
      ...
      test(
        'should default to pending when task current date is empty',
        () async {
      ...
      test(
        'should default to pending when task current date has invalid format',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            'invalid-date',
      ...
      test(
        'should default to pending when task current date has partial format',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            '2024-12',
      ...
      test(
        'should compute status correctly for dates far in the past',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            '2020-01-01',
      ...
      test(
        'should compute status correctly for dates far in the future',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            '2030-12-31',
      ...
      test(
        'should not interfere with existing task.currentStatus computation',
        () async {
          final today = DateTime.now();
          final yesterdayString = _formatDate(
    ...
    group('Task End Date Computation', () {
      test(
        'should compute isPastEndDate as false when end date equals today',
        () async {
          final today = DateTime.now();
          final todayString = _formatDate(today);
          ...
          final isPastEndDate = await userDataService.getValue<bool>(
      ...
      test(
        'should compute isPastEndDate as true when end date is before today',
        () async {
          final today = DateTime.now();
          final yesterday = today.subtract(const Duration(days: 1));
          final yesterdayString = _formatDate(yesterday);
      ...
      test(
        'should compute isPastEndDate as false when end date is after today',
        () async {
          final today = DateTime.now();
          final tomorrow = today.add(const Duration(days: 1));
          final tomorrowString = _formatDate(tomorrow);
      ...
      test(
        'should default to false when task end date has invalid format',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskEndDate,
            'invalid-date',
      ...
      test(
        'should default to false when task end date has partial format',
        () async {
      ...
      test('should not interfere with existing boolean computations', () async {
        final today = DateTime.now();
        final twoDaysAgoString = _formatDate(
    ...
    group('Task End Date Computation', () {
      test(
        'should compute task.endDate as next day when active days are everyday',
        () async {
          ...
          final endDate = await userDataService.getValue<String>(
            StorageKeys.taskEndDate,
          );
      ...
      test(
        'should compute task.endDate as next Monday when active days are weekdays and task is Friday',
        () async {
          final friday = DateTime(2024, 1, 5); // Friday
      ...
      test(
        'should compute task.endDate as next Saturday when active days are weekends and task is Sunday',
        () async {
      ...
      test(
        'should compute task.endDate correctly with single active day',
        () async {
      ...
      test(
        'should default to empty string when no task.currentDate is set',
        () async {
          // Don't set any task date
          await userDataService.storeValue(StorageKeys.taskActiveDays, [
            1,
      ...
      test(
        'should default to empty string when task.currentDate is empty',
        () async {
          await userDataService.storeValue(StorageKeys.taskCurrentDate, '');
          await userDataService.storeValue(StorageKeys.taskActiveDays, [
            1,
      ...
      test(
        'should default to empty string when task.currentDate has invalid format',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            'invalid-date',
      ...
      test(
        'should default to next day when no activeDays configured',
        () async {
          final friday = DateTime(2024, 1, 5); // Friday
          final saturday = DateTime(2024, 1, 6); // Saturday
          final fridayString = _formatDate(friday);
      ...
      test(
        'should default to next day when activeDays is empty array',
        () async {
          final friday = DateTime(2024, 1, 5); // Friday
          final saturday = DateTime(2024, 1, 6); // Saturday
          final fridayString = _formatDate(friday);
      ...
      test('should be recalculable via DataAction trigger', () async {
        ...
        final initialEndDate = await userDataService.getValue<String>(
          StorageKeys.taskEndDate,
        );
        ...
        final newEndDate = await userDataService.getValue<String>(
          StorageKeys.taskEndDate,
        );
      ...
      test(
        'should recalculate task.isPastEndDate when recalculateTaskEndDate is called',
        () async {
          final yesterday = DateTime.now().subtract(const Duration(days: 1));
          final yesterdayString = _formatDate(yesterday);
          ...
          final initialIsPastEndDate = await userDataService.getValue<bool>(
            StorageKeys.taskIsPastEndDate,
          );
          ...
          final newIsPastEndDate = await userDataService.getValue<bool>(
            StorageKeys.taskIsPastEndDate,
          );
          ...
          expect(
            newEndDate,
            _formatDate(DateTime.now()),
          ); // Should be recalculated to today
          expect(
            newIsPastEndDate,
            false,
          ); // Should be false since endDate is now today
        },
      );
    });
    ...
    group('Task Due Day Computation', () {
      test(
        'should compute task.dueDay as weekday integer of task.currentDate',
        () async {
          ...
          final dueDay = await userDataService.getValue<int>(
      ...
      test(
        'should compute task.dueDay correctly for different weekdays',
        () async {
          final testCases = [
          ...
          for (final (date, expectedWeekday) in testCases) {
            ...
            expect(
              dueDay,
              expectedWeekday,
              reason: 'Failed for ${date.toString()}',
            );
          }
        },
      );
      ...
      test(
        'should default to 0 when task.currentDate has invalid format',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            'invalid-date',
      ...
      test(
        'should default to 0 when task.currentDate has partial format',
        () async {
          await userDataService.storeValue(
            StorageKeys.taskCurrentDate,
            '2024-12',
      ...
      test(
        'should compute correctly for dates in different months and years',
        () async {
          final testCases = [
      ...
      test(
        'should recalculate task.status when recalculateTaskStatus is called',
        () async {
          final yesterday = DateTime.now().subtract(const Duration(days: 1));
          final yesterdayString = _formatDate(yesterday);
          ...
          final initialStatus = await userDataService.getValue<String>(
            StorageKeys.taskStatus,
          );
          ...
          final newStatus = await userDataService.getValue<String>(
            StorageKeys.taskStatus,
          );
