  ...
  group('Comprehensive Sequence Validation', () {
    ...
    test('Validate all sequence files for structural integrity', () async {
      ...
      if (result.warnings.isNotEmpty) {
        ...
        for (final warning in result.warnings) {
          warningsByType[warning.type] =
              (warningsByType[warning.type] ?? 0) + 1;
        }
      ...
      expect(
        ...
        0,
        reason:
            'Validation failed with ${result.errors.length} errors. See output above for details.',
      );
    ...
    test('Validate asset file accessibility', () async {
      ...
      expect(
        ...
        0,
        reason:
            'Some asset files are not accessible. See output above for details.',
      );
    });
    ...
    test('Validate JSON schema compliance', () async {
      ...
      expect(
        ...
        0,
        reason:
            'JSON schema validation failed with ${allErrors.length} errors.',
      );
    });
    ...
    test('Performance test: Validation should complete quickly', () async {
      ...
      print(
        '  Avg Time per Sequence: ${duration / AppConstants.availableSequences.length}ms',
      );
    ...
    test('Validate each sequence individually', () async {
      ...
      for (final sequenceId in AppConstants.availableSequences) {
        try {
          ...
          final jsonResult = await assetValidator.validateJsonSchema(
            sequenceId,
          );
          ...
          expect(
            ...
            0,
            reason:
                'Sequence "$sequenceId" has ${jsonResult.errors.length} JSON errors',
          );
        } catch (e) {
