...
void main() {
  group('MessageDisplayManager', () {
    ...
    test('should allow repeated messages in conversation loops', () async {
      ...
      expect(
        displayManager.displayedMessages[0].text,
        equals('Do you want to try again?'),
      );
      expect(displayManager.displayedMessages[1].text, equals('Great choice!'));
      expect(
        displayManager.displayedMessages[2].text,
        equals('Do you want to try again?'),
      );
    ...
    test('should filter out empty messages that are not interactive', () async {
      ...
      expect(
        displayManager.displayedMessages[1].text,
        equals('Another valid message'),
      );
    });

    test(
      'should allow empty interactive messages (choices and text inputs)',
      () async {
        ...
        final messages = [
          ...
          ), // TextInput must have empty text
        ...
        expect(displayManager.displayedMessages.length, equals(3));
        expect(
          displayManager.displayedMessages[0].type == MessageType.choice,
          isTrue,
        );
        expect(
          displayManager.displayedMessages[1].type == MessageType.textInput,
          isTrue,
        );
        expect(
          displayManager.displayedMessages[2].text,
          equals('Regular message'),
        );
      },
    );
    ...
    test('should handle multiple identical messages in sequence', () async {
      ...
      expect(displayManager.displayedMessages.length, equals(3));
      expect(
        displayManager.displayedMessages.every(
          (msg) => msg.text == 'Are you ready?',
        ),
        isTrue,
      );
      expect(notificationCount, equals(3));
